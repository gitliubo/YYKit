    ...
    if (@available(iOS 17.0, *)) {
        ...
        image = [renderer imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {
            ...
            for (UIWindow *window in windows) {
                ...
                if ([window isKindOfClass:weakSelf.class]) break; //don't capture window above self
                CGContextSaveGState(context);
                CGContextConcatCTM(context, YYCGAffineTransformGetFromViews(window, weakSelf));
